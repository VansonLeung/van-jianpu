...
- Electrobun 2 desktop support replaces Electron in the third independent `desktop` package. Use the OS native webview with CEF/WGPU disabled and the bundled Bun runtime for Express/sharp. Bundle compiled frontend, soundfont, backend, and production dependencies. Keep an authenticated loopback origin on a remembered per-profile port for IndexedDB; sandbox the renderer, restrict navigation, and exclude development credentials. Support native exports to Downloads, crop windows, menus, local launches and host-specific packaging; preserve browser mode. Transfer old Electron projects through `.jianpu`/JSON import without deleting the old profile.
...
- Persist all pages, the active page, rectangles, order, and corrected/model text in IndexedDB. Support project renaming and use the name for TXT/SVG/project filenames. Export editable `.jianpu` ZIP archives with a version 3 JSON manifest and separate lossless image files; deduplicate identical images and process archives in a worker. Import existing version 1/2 JSON projects; keep IndexedDB storage compatible. Preserve original JPEG bytes and EXIF orientation instead of expanding photos into PNG. Page switching preserves per-line session undo and routes pending scan results to their original page.

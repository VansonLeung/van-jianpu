...
6. Export TXT or an editable `.jianpu.json` project. Text export follows page order, then line order, with a blank line between pages that contain lines; an empty line becomes `?`. The project contains all images, rectangles, order, and both edited/model text. Deleting a page also deletes its lines after confirmation.
...
Projects support up to 100 image pages, 200 lines per page, and 100 MB of embedded project data. A failed image batch leaves the existing project intact. Editable JSON uses version 2: `{ version: 2, id, activePageId, pages: [{ id, image, lines }] }`; page and line array order is reading order. Existing version 1 single-image JSON and locally saved projects migrate automatically to one page. The backend API still receives one cropped line per request.
